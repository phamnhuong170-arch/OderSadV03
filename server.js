...
//  Chay: node server.js
//  Env:  DATABASE_URL  (Render tu inject, khong can .env)
// ================================================================
...
if (!DB_URL) {
  console.error('[FATAL] DATABASE_URL chua duoc set.');
  console.error('        Render > Web Service > Environment > them DATABASE_URL');
  process.exit(1);
}
...
// ----------------------------------------------------------------
//  INIT DATABASE  (tao bang + seed lan dau)
// ----------------------------------------------------------------
async function initDB() {
  ...
  const { rows } = await sql('SELECT COUNT(*) AS c FROM articles');
  if (parseInt(rows[0].c) > 0) { console.log('[DB] Da co du lieu, bo qua seed.'); return; }
...
Nguoi choi ky cuu cho rang nhung thay doi lam mat tinh skill-intensive. Nguoi moi lai chao don vi game de tiep can hon.`,
  ...
  for (const s of seeds) {
    await sql(
      ...
      [s.title, s.excerpt, s.content, s.image_url, s.category, s.author, s.views, s.h]
    );
  }
...
// ----------------------------------------------------------------
//  FRONTEND  (toan bo HTML + CSS + JS trong 1 string)
// ----------------------------------------------------------------
...
<div class="topbar">
  <div class="topbar-inner">
    ...
    <span>//</span><strong>Xbox Series X giam gia manh tai Viet Nam</strong>
  </div>
</div>
  ...
  <div id="listView">
    <div class="hero-sec">
      <div class="hero-grid" id="heroGrid">
        <div class="spin-wrap" style="grid-column:1/-1"><div class="spin"></div></div>
      </div>
    </div>
    <div class="sec-title">
      <h2 id="secLabel">TIN MOI NHAT</h2>
      <div class="line"></div>
      <span class="stag" onclick="showHome()">Xem tat ca</span>
    </div>
    <div class="content-layout">
      <div>
        <div class="articles-grid" id="artGrid">
          <div class="spin-wrap" style="grid-column:1/-1"><div class="spin"></div></div>
        </div>
        <div class="load-wrap">
          <button class="btn-more" id="btnMore" onclick="loadMore()">Tai them bai viet</button>
        </div>
      </div>
  ...
  <div class="ftr">
    <div class="fgrid">
      ...
      <div class="fcol"><h4>Ho Tro</h4><ul>
        ...
        <li><a>Lien he</a></li>
        <li><a>Quang cao</a></li>
        <li><a>Dieu khoan</a></li>
        <li><a>Bao mat</a></li>
      </ul></div>
    </div>
    <div class="fbot">
      <p>2026 GameK.vn — Ban quyen thuoc ve GameK</p>
      <p>Phien ban 3.0</p>
    </div>
  </div>
</footer>
...
<div class="overlay" id="overlay" onclick="if(event.target===this)closeModal()">
  <div class="modal">
    ...
    <div class="modal-body">
      <div class="fg"><label>Tieu de *</label><input id="f_title" type="text" placeholder="Nhap tieu de hap dan..."></div>
...
const $post = (p,body) => fetch('/api'+p,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(async r=>{const d=await r.json();if(!r.ok)throw new Error(d.error||'Server error');return d;});
...
const img = (a) => \`src="\${a.image_url||fb(a.id)}" onerror="this.src='\${fb(a.id)}'" alt=""\`;
...
async function renderHero(){
  ...
  try{
    ...
    const [m,...s]=a;
    g.innerHTML=\`
      <div class="hero-main" onclick="openArticle(\${m.id})">
        <img \${img(m)}>
        <div class="hero-ov">
          <div class="cat">\${m.category}</div>
          <h2>\${m.title}</h2>
          <div class="meta">\${m.author} &nbsp;•&nbsp; \${ago(m.created_at)} &nbsp;•&nbsp; \${fv(m.views)} luot xem</div>
        </div>
      </div>
      <div class="hero-side">
        \${s.slice(0,2).map(x=>\`
          <div class="hsc" onclick="openArticle(\${x.id})">
            <img \${img(x)}>
            <div>
              <span class="cat">\${x.category}</span>
              <h3>\${x.title}</h3>
              <div class="meta">\${ago(x.created_at)} &nbsp;•&nbsp; \${fv(x.views)} luot xem</div>
            </div>
          </div>\`).join('')}
      </div>\`;
...
async function renderArticles(append=false){
  ...
  try{
    ...
    if(!articles.length&&!append){
      g.innerHTML='<div class="empty"><h3>Chua co bai viet</h3><p>Danh muc nay chua co noi dung.</p></div>';
      return;
    }
    ST.offset+=articles.length;
    articles.forEach((a,i)=>{
      const c=document.createElement('div');
      c.className='card';c.style.animationDelay=(i*.08)+'s';c.onclick=()=>openArticle(a.id);
      c.innerHTML=\`
        <div class="card-img"><img \${img(a)}></div>
        <div class="card-body">
          <span class="cat">\${a.category}</span>
          <h3>\${a.title}</h3>
          <p class="exc">\${a.excerpt}</p>
          <div class="meta"><span>\${a.author}</span><span>\${ago(a.created_at)}</span><span>\${fv(a.views)} luot xem</span></div>
        </div>\`;
      g.appendChild(c);
    });
    const btn=document.getElementById('btnMore');
    btn.disabled=ST.offset>=ST.total;
    btn.textContent=ST.offset>=ST.total?'Da tai het':'Tai them bai viet';
  }catch(e){
    if(!append) g.innerHTML='<div class="empty"><h3>Loi ket noi</h3><p>'+e.message+'</p></div>';
...
async function renderSidebar(id='sidebar'){
  ...
  try{
    ...
    el.innerHTML=\`
      <div class="widget">
        <div class="wtitle">BAI VIET HOT NHAT</div>
        <div class="hot-list">
          \${tr.map((a,i)=>\`
            <div class="hot-item" onclick="openArticle(\${a.id})">
      ...
      <div class="widget">
        <div class="wtitle">CHU DE NOI BAT</div>
        <div class="tag-cloud">
          \${tags.map(t=>\`<span class="tag" onclick="doSearchTag('\${t}')">\${t}</span>\`).join('')}
        </div>
      </div>\`;
...
async function openArticle(id){
  ...
  try{
    ...
    const body=a.content.split('\\n').filter(l=>l.trim()).map(l=>
      l.startsWith('### ')?'<h3>'+l.slice(4)+'</h3>':'<p>'+l+'</p>'
    ).join('');
    dv.innerHTML=\`
      <div class="content-layout">
        <div>
          <div class="d-bc"><a onclick="showHome()">Trang chu</a> / <span>\${a.category}</span></div>
          <div class="d-cat">\${a.category}</div>
          <h1 class="d-title">\${a.title}</h1>
          <div class="d-meta">
            <span>Tac gia: <strong>\${a.author}</strong></span>
            <span>\${ago(a.created_at)}</span>
            <span>\${fv(a.views)} luot xem</span>
          </div>
          <img class="d-img" \${img(a)}>
          <div class="d-body">\${body}</div>
          <div style="margin-top:24px;display:flex;gap:8px;flex-wrap:wrap">
            \${[a.category,'Game','Tin Tuc'].map(t=>\`<span class="tag">\${t}</span>\`).join('')}
          </div>
          <div id="relSec"></div>
        </div>
        <aside id="dSidebar"></aside>
      </div>\`;
    renderSidebar('dSidebar');
    // Related articles
    $get('/articles?category='+encodeURIComponent(a.category)+'&limit=6').then(({articles:rel})=>{
      const r=rel.filter(x=>x.id!==a.id).slice(0,3);
      if(!r.length)return;
      document.getElementById('relSec').innerHTML=\`
        <div class="sec-title" style="margin-top:36px">
          <h2>BAI VIET LIEN QUAN</h2><div class="line"></div>
        </div>
        \${r.map(x=>\`
          <div class="lcard" onclick="openArticle(\${x.id})">
            <img \${img(x)}>
            <div>
              <h3>\${x.title}</h3>
              <div class="meta">\${x.author} &nbsp;•&nbsp; \${ago(x.created_at)}</div>
            </div>
          </div>\`).join('')}\`;
    }).catch(()=>{});
  }catch(e){
    dv.innerHTML='<div class="empty"><h3>Loi tai bai viet</h3><p>'+e.message+'</p></div>';
  }
}
...
async function doSearch(){
  ...
  try{
    ...
    g.innerHTML='';
    if(!articles.length){
      g.innerHTML='<div class="empty"><h3>Khong co ket qua</h3><p>Khong tim thay ket qua nao cho "'+q+'"</p></div>';
      return;
    }
    ...
    articles.forEach((a,i)=>{
      ...
      c.innerHTML=\`
        ...
        <div class="card-body">
          ...
          <h3>\${a.title}</h3>
          <p class="exc">\${a.excerpt}</p>
          <div class="meta"><span>\${a.author}</span><span>\${ago(a.created_at)}</span></div>
        </div>\`;
      g.appendChild(c);
    });
    document.getElementById('btnMore').style.display='none';
  }catch(e){
    g.innerHTML='<div class="empty"><h3>Loi tim kiem</h3><p>'+e.message+'</p></div>';
  }
}
...
async function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      raw += chunk;
      if (raw.length > 5_000_000) reject(new Error('Payload qua lon'));
    });
...
async function router(req, res) {
  ...
  if (M === 'GET' && U === '/api/health')
    return ok(res, { status: 'ok', env: NODE_ENV, time: new Date().toISOString() });
  ...
  if (M === 'GET' && U.startsWith('/api/articles/trending')) {
    try {
      ...
      const { rows } = await sql('SELECT * FROM articles ORDER BY views DESC LIMIT $1', [limit]);
  ...
  if (M === 'GET' && U.startsWith('/api/articles/search')) {
    try {
      ...
      return ok(res, { articles: rows, total: rows.length });
    } catch(e) { return err(res, 500, e.message); }
  }
  ...
  if (M === 'GET' && /^\/api\/articles(\?|$)/.test(U)) {
    try {
      const { limit, offset, category } = qs(U);
      let rows, total;
      if (category) {
        ({ rows } = await sql(
          'SELECT * FROM articles WHERE category=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3',
          [category, limit, offset]
        ));
        ({ rows: [{ total }] } = await sql(
          'SELECT COUNT(*) AS total FROM articles WHERE category=$1', [category]
        ));
      } else {
        ({ rows } = await sql(
          'SELECT * FROM articles ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]
        ));
        ({ rows: [{ total }] } = await sql('SELECT COUNT(*) AS total FROM articles'));
      }
      return ok(res, { articles: rows, total: parseInt(total) });
    } catch(e) { return err(res, 500, e.message); }
  }
  ...
  if (M === 'GET' && mGet) {
    try {
      const { rows } = await sql('SELECT * FROM articles WHERE id=$1', [+mGet[1]]);
      if (!rows.length) return err(res, 404, 'Khong tim thay bai viet');
      sql('UPDATE articles SET views=views+1 WHERE id=$1', [rows[0].id]).catch(() => {});
      return ok(res, { article: rows[0] });
    } catch(e) { return err(res, 500, e.message); }
  }
  ...
  if (M === 'POST' && U === '/api/articles') {
    try {
      ...
      const { rows } = await sql(
        `INSERT INTO articles(title,excerpt,content,image_url,category,author)
         VALUES($1,$2,$3,$4,$5,$6) RETURNING *`,
        [b.title.trim(), b.excerpt||'', b.content.trim(), b.image_url||'', b.category, b.author||'Bien Tap Vien']
      );
      res.writeHead(201, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ success: true, article: rows[0] }));
    } catch(e) { return err(res, 500, e.message); }
  }
  ...
  if (M === 'PUT' && mPut) {
    try {
      const id = +mPut[1];
      const { rows: ex } = await sql('SELECT * FROM articles WHERE id=$1', [id]);
      if (!ex.length) return err(res, 404, 'Khong tim thay bai viet');
      const b = await readBody(req), a = ex[0];
      const { rows } = await sql(
        `UPDATE articles
         SET title=$1,excerpt=$2,content=$3,image_url=$4,category=$5,author=$6,updated_at=NOW()
         WHERE id=$7 RETURNING *`,
        [b.title??a.title, b.excerpt??a.excerpt, b.content??a.content,
         b.image_url??a.image_url, b.category??a.category, b.author??a.author, id]
      );
      return ok(res, { article: rows[0] });
    } catch(e) { return err(res, 500, e.message); }
  }
  ...
  if (M === 'DELETE' && mDel) {
    try {
      const { rowCount } = await sql('DELETE FROM articles WHERE id=$1', [+mDel[1]]);
      if (!rowCount) return err(res, 404, 'Khong tim thay bai viet');
